...
type KitsuAnime = { id: string; attributes?: { canonicalTitle?: string; titles?: { en?: string; en_jp?: string; ja_jp?: string }; synopsis?: string; posterImage?: { large?: string; original?: string }; showType?: string; status?: string; startDate?: string; episodeCount?: number; averageRating?: string }; relationships?: { mappings?: { data?: { id?: string }[] } } };
type KitsuMapping = { id: string; type: "mappings"; attributes?: { externalSite?: string; externalId?: string }; relationships?: { item?: { data?: { id?: string } } } };
async function searchKitsu(search: string, page = 1, genre?: string) { const limit = 20; const params = new URLSearchParams({ "filter[text]": search, "page[limit]": String(limit), "page[offset]": String((page - 1) * limit), include: "mappings" }); const response = await fetch(`${KITSU_API}/anime?${params}`); if (!response.ok) throw new Error(`Kitsu unavailable (${response.status})`); const json = await response.json() as { data: KitsuAnime[]; included?: KitsuMapping[]; meta?: { count?: number } }; const mappings = json.included || []; const items = json.data.map((item) => { const mappingIds = new Set(item.relationships?.mappings?.data?.map((mapping) => mapping.id)); const mapping = mappings.find((entry) => mappingIds.has(entry.id) && entry.attributes?.externalSite === "anilist/anime"); if (!mapping?.attributes?.externalId) return null; const value = item.attributes || {}; return { id: Number(mapping.attributes.externalId), title: { english: value.titles?.en || value.canonicalTitle, romaji: value.titles?.en_jp, native: value.titles?.ja_jp }, description: value.synopsis, coverImage: { extraLarge: value.posterImage?.large || value.posterImage?.original }, genres: [], format: value.showType || "TV", status: value.status === "finished" ? "FINISHED" : value.status === "upcoming" ? "NOT_YET_RELEASED" : "RELEASING", seasonYear: value.startDate ? new Date(value.startDate).getFullYear() : undefined, episodes: value.episodeCount, averageScore: value.averageRating ? Math.round(Number(value.averageRating)) : undefined } as Anime; }).filter((anime): anime is Anime => anime !== null && (!genre || anime.genres.some((item) => item.toLowerCase() === genre.toLowerCase()))); const count = json.meta?.count || items.length; return { items, pageInfo: { currentPage: page, lastPage: Math.max(page, Math.ceil(count / limit)), hasNextPage: page * limit < count } }; }
...
export async function searchAnime(search: string, page = 1, genre?: string) { if (!search.trim()) return { items: [], pageInfo: { currentPage: 1, lastPage: 1, hasNextPage: false } }; const term = search.trim(); try { return await searchKitsu(term, page, genre); } catch { try { return await searchJikan(term, page, genre); } catch { try { const data = await query<{ page: { media: Anime[]; pageInfo: PageInfo } }>(`query($search: String, $page: Int, $genre: String) { page: Page(page: $page, perPage: 24) { pageInfo { currentPage lastPage hasNextPage } media(search: $search, genre: $genre, sort: POPULARITY_DESC, type: ANIME, isAdult: false) { ${fields} } } }`, { search: term, page, genre: genre || undefined }); return { items: safe(data.page.media), pageInfo: data.page.pageInfo }; } catch { const lower = term.toLowerCase(); return { items: fallback.filter((a) => titleOf(a).toLowerCase().includes(lower) || a.genres.some((g) => g.toLowerCase().includes(lower))), pageInfo: { currentPage: 1, lastPage: 1, hasNextPage: false } }; } } } }
