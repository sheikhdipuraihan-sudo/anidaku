...
async function query<T>(queryText: string, variables: Record<string, unknown> = {}): Promise<T> { const key = JSON.stringify({ queryText, variables }); const hit = cache.get(key); if (hit && Date.now() - hit.at < CACHE_MS) return hit.value as T; const controller = new AbortController(); const timeout = window.setTimeout(() => controller.abort(), 5000); const response = await fetch(API, { method: "POST", headers: { "Content-Type": "application/json", Accept: "application/json" }, body: JSON.stringify({ query: queryText, variables }), signal: controller.signal }); window.clearTimeout(timeout); if (!response.ok) throw new Error(`AniList unavailable (${response.status})`); const json = await response.json(); if (json.errors?.length) throw new Error("AniList returned an error"); cache.set(key, { at: Date.now(), value: json.data }); return json.data as T; }
