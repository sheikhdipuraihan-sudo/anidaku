...
type KitsuAnime = { id: string; attributes?: { canonicalTitle?: string; titles?: { en?: string; en_jp?: string; ja_jp?: string }; synopsis?: string; posterImage?: { large?: string; original?: string }; showType?: string; status?: string; startDate?: string; episodeCount?: number; averageRating?: string } };
async function searchKitsu(search: string, page = 1, genre?: string) { const limit = 20; const params = new URLSearchParams({ "filter[text]": search, "page[limit]": String(limit), "page[offset]": String((page - 1) * limit) }); const response = await fetch(`${KITSU_API}/anime?${params}`); if (!response.ok) throw new Error(`Kitsu unavailable (${response.status})`); const json = await response.json() as { data: KitsuAnime[]; meta?: { count?: number } }; const items = json.data.map((item) => { const value = item.attributes || {}; return { id: Number(item.id), title: { english: value.titles?.en || value.canonicalTitle, romaji: value.titles?.en_jp, native: value.titles?.ja_jp }, description: value.synopsis, coverImage: { extraLarge: value.posterImage?.large || value.posterImage?.original }, genres: [], format: value.showType || "TV", status: value.status === "finished" ? "FINISHED" : value.status === "upcoming" ? "NOT_YET_RELEASED" : "RELEASING", seasonYear: value.startDate ? new Date(value.startDate).getFullYear() : undefined, episodes: value.episodeCount, averageScore: value.averageRating ? Math.round(Number(value.averageRating)) : undefined } as Anime; }).filter((anime) => !genre || anime.genres.some((item) => item.toLowerCase() === genre.toLowerCase())); const count = json.meta?.count || items.length; return { items, pageInfo: { currentPage: page, lastPage: Math.max(page, Math.ceil(count / limit)), hasNextPage: page * limit < count } }; }
